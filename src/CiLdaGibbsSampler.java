/*
 * CiLdaGibbsSampler is free software; you can redistribute it and/or modify it
 ...
 *
 */
public class CiLdaGibbsSampler {
     ...
     * @param data
     */
    public CiLdaGibbsSampler(int[][] documentWords, int[][] documentEntities, int W, int E) {
     ...
     * zw_-i, w, ze) = (cwt_-i,j(w_i) + betaw)/(sumcwt_-i,j(.) + W * betaw) * (ctd_-i,j(d_i) +
     ...
     * ze_-i, e, zw) = (cet_-i,j(w_i) + betae)/(sumcet_-i,j(.) + E * betae) * (ctd_-i,j(d_i) +
     * alpha)/(sumcet_-i,.(d_i) + K * alpha)
     * 
     * @param m
     *            document
     * @param n
	...
	if (args.length != 8)
        {
            System.err.println("\nUSAGE: java CiLdaGibbsSampler <documentWordIndexFile> <wordVocabFile> <documentEntityIndexFile> <entityVocabFile> <K> <alpha> <betaw> <betae>\n");
            System.exit(0);
        }
        ...
        try
    	{
			...
			System.out.println("\nCI Latent Dirichlet Allocation using Gibbs Sampling.");
			System.out.println("\nTraining " + K + " CiLDA topics for " + M + " documents with " + W + " words vocabulary and " + E + " entities vocabulary ...\n");

			CiLdaGibbsSampler cilda = new CiLdaGibbsSampler(documentWords, documentEntities, W, E);
			cilda.configure(2500, 1000, 100, 10);
			cilda.gibbs(K, alpha, betaw, betae);

			double[][] theta = cilda.getTheta();
			double[][] phiw = cilda.getPhiW();
			double[][] phie = cilda.getPhiE();
			int[][] cwt = cilda.getCwt();
			int[][] cet = cilda.getCet();

			TopicUtils.saveCSV(theta,"./CiLDA_theta");
			TopicUtils.saveCSV(phiw,"./CiLDA_phiw");
			TopicUtils.saveCSV(phie,"./CiLDA_phie");
			TopicUtils.saveCSV(cwt,"./CiLDA_cwt");
			TopicUtils.saveCSV(cet,"./CiLDA_cet");
			TopicUtils.saveTopTopicWords(phiw, wcr.getVocab(), "./CiLDA_top-topic-words", 10);
			TopicUtils.saveTopTopicWords(phie, ecr.getVocab(), "./CiLDA_top-topic-entities", 10);
